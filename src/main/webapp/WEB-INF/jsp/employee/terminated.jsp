...
<head>
	...
	<title><spring:message code="BzComposer.terminatedemployeelisttitle"/>
	</title>
	...
	<script type="text/javascript">
		...
		function getEmployeeInfo(employeeid,rowId)
		{
			...
			window.location = "employeelist?type=terminatedDetails&emid="+employeeid;
		}
	...
	<script>
		$(function() {
			$('#sortBy').change(function(){
				...
				$.ajax({
					type: "POST",
					url:"EmployeeAjax?tabid=sortTerminated&SortBy="+sortBy,
					data:{sortBy : sortBy},
				}).done(function(data){
					$('#custTableBody').html('');
					var custDetails = "";
					for(var i=0; i<data.length; i++){
						...
						custDetails = custDetails +"<input type='hidden' name='listSize' id='lSize' value='"+data.length+"'>" +
								"<tr id='"+i+"$$' onclick='setRowId("+objList.employeeID+","+i+")'>" +
