	...
	<div id="cos">
		<div class="statusquo ok">
						...
						<div>
							<div style="float: left;">
								<span
									...
									<spring:message code="BzComposer.customer.caseBoard" /> <c:if
            ...
            %>
										...
										<tbody><tr>
											<td style="width: 30%; padding: 10px;">
												<table style="width: 100%; font-size: 14px; border-right: 2px solid #dddddd;" cellpadding="5">
													...
													<tr>
														<td>Date Opened From :</td>
														<td>Date Opened To :</td>
													</tr>
													<tr>
											...
											<td style="width: 20%;">
												<div>
													<button type="button" class="formbutton" onclick="CaseSearch(1);" style="width: 70px;">
														Search
													</button>
												</div>
												<div>
													<button type="button" class="formbutton" onclick="CaseSearch(2);" style="width: 70px; margin-top: 10px;">
														Refresh
													</button>
												</div>
												<div>
													<button type="button" class="formbutton" onclick="CaseSearch(3);" style="width: 70px; margin-top: 10px; margin-right: 20px;">
														Clear
													</button>
												</div>
											</td>
						...
						<input type="hidden" id="lSize" value='${caseList.size()}' />
								...
								<tr valign="center">
									<th style="padding-right: 30px !important;"><spring:message
															...
															<th><spring:message
																	...
																	<th><spring:message
																	code="BzComposer.customer.case.dateClosed" /></th>
							...
							<tbody id="custTableBody">
								<c:if test="${not empty caseList}">
															<c:forEach items="${caseList}" var="objList"
																varStatus="loop">
																<tr id='${loop.index}$$' onclick="setRowId(${objList.caseID}, ${loop.index},  true);"
																ondblclick="goToCaseManager(${objList.caseID});" >

																	<td><input type="checkbox" id="custId${loop.index}"
...
function CaseSearch(filterType)
{
	if(filterType > 1)
	{
        location.reload();
    ...
    var obj=JSON.stringify(caseSearchDetails);
    
   
    
    $.ajax({
        type : "POST",
        url : "/AllCases?tabid=FilterList",
        data:"data=" +obj,
...
function goToCaseManager()
{

	window.location="Case?tabid=caseManager&caseID="+itemID;	
}
function initialize(){
...
function addNewCase()
{
	...
	window.open("Case?tabid=caseBoardNewCase",null,"scrollbars=yes,height=700,width=1250,status=yes,toolbar=no,menubar=no,location=no");
...
function editCase() 
{
	...
	if(confirm("<spring:message code='BzComposer.customer.selectAtOneEle'/>")==true)
	 return false;
       ...
       window.open("Case?tabid=caseBoardEditCase&caseID="+itemID, null,"scrollbars=yes,height=700,width=1250,status=yes,toolbar=no,menubar=no,location=no");
...
function openMailSender(){
    if (selectedRowIndexs.length == 0){
        alert("<spring:message code='BzComposer.printlabels.selectCase'/>");
        return false;
    }else{
        let CustIDs = "";
        for(let x=0; x<selectedRowIndexs.length; x++){
        ...
        window.open("Customer?tabid=ShowSendEmailOnCaseBoard&contactIDs="+CustIDs, null,"scrollbars=yes,height=450,width=800,status=yes,toolbar=no,menubar=no,location=no");
    }
}
