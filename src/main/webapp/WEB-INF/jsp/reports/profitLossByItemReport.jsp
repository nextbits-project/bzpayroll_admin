...
<title>${sessionScope.user} - <spring:message code="BzComposer.Report.Profit&Budget.ProfitLossByItem"/></title>
<style>
...
<body>
<form:form action="Item?tabid=ProfitLossByItem" method="post" modelAttribute="itemDto">
...
<div id="printContent">	
	<div id="headerBar">
	      ...
	      <h6 style="text-align: center;color: blue;" id="headerBarValue"><spring:message code="BzComposer.Report.Profit&Budget.ProfitLossByItem"/></h6>
	</div>
	...
	<table class="tabla-customListOds" cellspacing="0" id="exportPd">
		<thead>
			<tr>
					<th style="padding-left: 50px"><spring:message code="BzComposer.Invoice.ItemID" /></th>
					<th style="padding-left: 50px"><spring:message code="BzComposer.Report.ReservedInventoryList.InventoryCode" /></th>
					<th style="padding-left: 50px"><spring:message code="BzComposer.Item.SalePriceEachItem" /></th>
					<th style="padding-left: 50px"><spring:message code="BzComposer.Item.TotalSalePrice" /></th>
			</tr>
		</thead>
		<tbody>
            <c:if test="${not empty profitLossByItem}">
                <input type="hidden" name="sListSize" id="lSize" value='${profitLossByItem.size()}'>
                <c:forEach items="${profitLossByItem}" var="curObject">
                    <tr>
                    <td style="padding-left: 50px">${curObject.inventoryId}</td>
                    <td style="padding-left: 50px">${curObject.inventoryCode}</td>
                    <td style="padding-left: 50px">${curObject.salePrice}</td>
                    <td style="padding-left: 50px">${curObject.totalSaleprice}</td>
                    </tr>
                </c:forEach>
                <tr>
                    <td style="color: blue;padding-left: 50px"><b><spring:message code="BzComposer.Invoice.Total"/></b></td>
                    <td></td>
                    <td></td>
                    <td style="color: blue;padding-left: 50px">${TotalGrossprofit}"</td>
                </tr>
            </c:if>
		</tbody>
	</table>
