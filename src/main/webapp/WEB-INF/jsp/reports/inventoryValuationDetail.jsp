...
<title>${sessionScope.user} - <spring:message code="BzComposer.Valuation.InventoryValuation.Detail"/></title>
<style>
...
<body>
<form:form action="Item?tabid=InvValDetail" method="post" modelAttribute="itemDto">
	...
	<div id="headerBar">
          ...
          <h6 style="text-align: center;color: blue;" id="headerBarValue"><spring:message code="BzComposer.Valuation.InventoryValuation.Detail"/></h6>
    </div>
	...
	<table class="tabla-customListOds" id="exportPd" border="1">
		...
		<tbody>
			<c:if test="${not empty getInvValDetail}">
                <input type="hidden" name="sListSize" id="lSize" value='${getInvValDetail.size()}'>
                <c:forEach items="${getInvValDetail}" var="curObject">
                    <tr>
                        <td align="left">${curObject.category}</td>
                        <td align="left">${curObject.cvName}</td>
                        <td align="left">${curObject.itemType}</td>
...
function search()
{
	document.forms[0].action = "Item?tabid=InvValDetail";
