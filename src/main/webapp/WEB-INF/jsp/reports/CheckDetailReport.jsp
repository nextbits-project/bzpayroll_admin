...
<title>${sessionScope.user}-<spring:message code="BzComposer.Report.Banking&Accounting.CheckDetail"/></title>
<style>
...
<body>
<form:form action="BankingAccounting?tabid=CheckDetail" method="post" modelAttribute="accountDto">
...
<div id="printContent">
	...
	<div id="headerBar">
        ...
        <h6 style="text-align: center; color: blue;"
            id="headerBarValue">
            <spring:message code="BzComposer.Report.Banking&Accounting.CheckDetail" />
        </h6>
    </div>
    ...
    <table class="tabla-customListOds" id="exportPd" border="1">
		<thead>
			<tr>
					...
					<th><spring:message code="BzComposer.Report.Banking&Accounting.AccountName" /></th>
					<th ><spring:message code="BzComposer.Invoice.Date.DateAdded" /></th>
					<th ><spring:message code="BzComposer.InvoiceId" /></th>
					<th ><spring:message code="BzComposer.Report.Amount" /></th>
			</tr>
		</thead>
		<tbody>
            <c:if test="${not empty checkDetail}">
                <input type="hidden" name="sListSize" id="lSize" value='${checkDetail.size()}'>
                <c:forEach items="${checkDetail}" var="curObject">
                    <tr>
                        <td>${curObject.chekcno}</td>
                        <td>${curObject.bankName}</td>
                        <td>${curObject.date}</td>
                        <td>${curObject.invoiceId}</td>
...
function search()
{
	document.forms[0].action = "BankingAccounting?tabid=CheckDetail";
