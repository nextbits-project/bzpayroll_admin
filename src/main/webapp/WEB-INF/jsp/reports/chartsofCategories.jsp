...
<title>${sessionScope.user} - <spring:message code="BzComposer.Report.Lists.chart"/></title>
<style>
...
<body>
<form:form action="ReportCenterLists?tabid=ChartsofCategories" method="post" modelAttribute="accountDto">
...
<div id="headerBar">
	      ...
	      <h6 style="text-align: center;color: blue;" id="headerBarValue"><spring:message code="BzComposer.Report.Lists.chart"/></h6>
	</div>
	<div id="table-negotiations" style="overflow:auto;height:500; text-align: center;">
		<table class="tabla-customListOds" id="exportPd" border="1">
			<thead>
				<tr>
						<th><spring:message code="CategoryList.Category_Type" /></th>
						<th><spring:message code="CategoryList.Name" /></th>
						<th><spring:message code="CategoryList.Number" /></th>
						<th><spring:message code="CategoryList.Description" /></th>
						<th><spring:message code="CategoryList.BudgetCategory" /></th>	
				</tr>
			</thead>
			<tbody>	
				<c:forEach items="${acList}" var="curObject">
				<tr>
					<td>${curObject.categoryTypeName}</td>
					<td>${curObject.categoryName}</td>
					<td>${curObject.cateNumber}</td>
					<td>${curObject.catedescription}</td>
					<td>${curObject.budgetCateName}</td>
				</tr>
				</c:forEach>
