...
<body>
<form:form action="SalesBord?tabid=SalesRBI&ilist=2" method="post" modelAttribute="salesBoardDto">
...
<div id="printContent">	
	<div id="headerBar">
	      ...
	      <h6 style="text-align: center;color: blue;" id="headerBarValue"><spring:message code="BzComposer.SalesPanel.SalesReportByItem"/></h6>
	</div>
	<div id="table-negotiations" style="overflow:auto;height:400; text-align: center;">
		...
		<thead>
			<tr>
				...
				<th><spring:message code="BzComposer.Report.ReservedInventoryList.SoldQty" /></th>
				<th><spring:message code="BzComposer.Report.ReservedInventoryList.SoldAmt" /></th>
				<th><spring:message code="BzComposer.Report.Employee.SalesReportByRep.RefundQty" /></th>
				<th><spring:message code="BzComposer.Report.Employee.SalesReportByRep.RefundAmount" /></th>
			</tr>
		</thead>

		<tbody>
			<c:if test="${not empty salesRBC}">
<%-- 		<input type="hidden" name="sListSize" id="lSize" value='${catList}'> --%>
    		<c:forEach items="${salesRBC}" var="sale">
                <tr>
                <td>${sale.inventoryCode}</td>
                <td>${sale.soldQty}</td>
                <td>${sale.soldAmount}</td>
                <td>${sale.refundQty}</td>
                <td>${sale.refundAmt}</td>
                </tr>
			</c:forEach>
			</c:if>
		</tbody>
...
function search()
{
	document.forms[0].action = "SalesBord?tabid=SalesRBI&ilist=2";
