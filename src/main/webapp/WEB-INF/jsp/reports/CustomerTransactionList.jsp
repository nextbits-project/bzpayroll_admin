...
<title>${sessionScope.user} - <spring:message code="BzComposer.reportcenter.listing.transactionlistbycustomer"/></title>
<style>
table.tabla-customListOds { width: 90%; border: 1px solid rgb(207, 207, 207); margin: 5px; }
...
<form:form action="Customer?tabid=CustomerTransactionList" method="post" modelAttribute="customerDto">
...
<div id="printContent">	
	<div id="headerBar">
	      ...
	      <h6 style="text-align: center;color: blue;" id="headerBarValue"><spring:message code="BzComposer.Report.Transactionlist.ByCustomer"/></h6>
	</div>
	<div id="table-negotiations"
		style="overflow:auto;height:500; text-align: center;">
		...
		<thead>
			...
			<th><spring:message code="BzComposer.Invoice.Num" /></th> 
		...
		<tbody>
            <c:if test="${not empty customerTransactionList}">
<%-- 			<input type="hidden" name="sListSize" id="lSize" value='${customerTransactionList.size()}'> --%>
            <c:forEach items="${customerTransactionList}" var="customer">
                <tr>
...
function search()
{
	document.forms[0].action = "Customer?tabid=CustomerTransactionList";
