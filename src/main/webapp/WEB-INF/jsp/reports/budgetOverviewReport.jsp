...
<title>${sessionScope.user} - BzComposer - Budget Overview Report</title>
<style>
...
<div id="printContent">	
	<div id="headerBar">
	      ...
	      <h6 style="text-align: center;color: blue;" id="headerBarValue">Budget Overview Report</h6>
	</div>
	<div id="table-negotiations" style="overflow:auto;height:400; text-align: center;">
		...
		<thead>
			<tr>
				...
				<th><spring:message code="BzComposer.Vendor.Memo" /></th>
				<th><spring:message code="BzComposer.Item.Qty" /></th>
...
<%-- 			<input type="hidden" name="sListSize" id="lSize" value='${salesRBC.size()}'> --%>
 				<c:forEach items="${salesRBC}" var="curObject">
                    <tr>
                    <td>${curObject.cvName}</td>
                    <td></td>
                    <td>${curObject.inventoryQty}</td>
                    <td>${curObject.amount}</td>
                    <td>${curObject.balTotal}</td>
                    </tr>
				</c:forEach>
			</c:if>
		</tbody>
