...
<title>${sessionScope.user}-<spring:message code="BzComposer.Report.PaidPurchaseBills"/></title>
<style>
...
<body>
<form:form action="PurchaseBoard?tabid=PaidPurchaseBillList" method="post" modelAttribute="purchaseBoardDto">
...
<div id="printContent">	
	...
	<div id="headerBar">
        ...
        <h6 style="text-align: center; color: blue;"
            id="headerBarValue">
            <spring:message code="BzComposer.Report.PaidPurchaseBills" />
        </h6>
    </div>
	...
	<table class="tabla-customListOds" id="exportPd" border="1">
		<thead>
			<tr>
					<th ><spring:message code="ReportCenter.Paid_Purchase_Bills.Date" /></th>
					<th><spring:message code="ReportCenter.Paid_Purchase_Bills.Client/Vendor" /></th>
					<th ><spring:message code="ReportCenter.Paid_Purchase_Bills.Rep" /></th>
					<th ><spring:message code="ReportCenter.Paid_Purchase_Bills.Amount" /></th>
					</tr>
		</thead>
		<tbody>
		    <c:if test="${not empty PurchaseBillDetails}">
                <input type="hidden" name="sListSize" id="lSize" value='${PurchaseBillDetails.size()}'>
                <c:forEach items="${PurchaseBillDetails}" var="curObject">
                    <tr>
                        <td>${curObject.po_no}</td>
                        <td>${curObject.lastName},${curObject.firstName}</td>
...
function search()
{
	document.forms[0].action = "PurchaseBoard?tabid=PaidPurchaseBillList";
