...
<title>${sessionScope.user} - <spring:message code="BzComposer.InventoryOrderReport"/></title>
<style>
...
<body>
<form:form action="Item?tabid=InvOrderReport" method="post" modelAttribute="itemDto">
	...
	<div id="headerBar">
          ...
          <h6 style="text-align: center;color: blue;" id="headerBarValue"><spring:message code="BzComposer.InventoryOrderReport"/></h6>
    </div>
	...
	<table class="tabla-customListOds" id="exportPd" border="1">
		<thead>
			<tr>
				...
				<th><spring:message code="BzComposer.Item.SalePrice" /></th>
				<th><spring:message code="BzComposer.purchase.PONumPrefix" /></th>
			</tr>
		</thead>
		<tbody>
		    <c:if test="${not empty invOrderReport}">
                <input type="hidden" name="sListSize" id="lSize" value='${invOrderReport.size()}'>
                <c:forEach items="${invOrderReport}" var="curObject">
                    <tr>
                        <td align="left">${curObject.category}</td>
                        <td align="left">${curObject.invName}</td>
                        <td align="left">${curObject.salePrice}</td>
                        <td>0</td>
                    </tr>
                </c:forEach>
            </c:if>
...
function search()
{
	document.forms[0].action = "Item?tabid=InvOrderReport";
