...
<title>${sessionScope.user} - <spring:message code="BzComposer.Report.Sales.SalesByCustomerSummary"/></title>
<style>
...
<body>
<form:form action="Customer?tabid=SalesByCustomerSummary" method="post" modelAttribute="customerDto">
...
<div id="printContent">	
	<div id="headerBar">
	      ...
	      <h6 style="text-align: center;color: blue;" id="headerBarValue"><spring:message code="BzComposer.Report.Sales.SalesByCustomerSummary"/></h6>
	</div>
	<div id="table-negotiations" style="overflow:auto;height:400; text-align: center;">
		...
		<tbody>
			<c:if test="${not empty getSalesCustomerSummary}">
<%-- 	    <input type="hidden" name="sListSize" id="lSize" value='${getSalesCustomerSummary.size()}'>  --%>
            <c:forEach items="${getSalesCustomerSummary}" var="customer">
                    <tr>
...
function search()
{
	document.forms[0].action = "Customer?tabid=SalesByCustomerSummary";
