...
<title>${sessionScope.user} - <spring:message code="BzComposer.Inventory.CurrentInventoryStatictics"/></title>
<style>
...
<body>
<form:form action="Item?tabid=InvStatistic" method="post" modelAttribute="itemDto">
	...
	<div id="headerBar">
          ...
          <h6 style="text-align: center;color: blue;" id="headerBarValue"><spring:message code="BzComposer.Inventory.CurrentInventoryStatictics"/></h6>
    </div>
	...
	<table class="tabla-customListOds" id="exportPd" border="1">
		<thead>
			<tr>
				...
				<th><spring:message code="BzComposer.Item.ItemCode" /></th>
				<th><spring:message code="BzComposer.Invoice.ItemName" /></th>
				<th><spring:message code="BzComposer.Item.Location" /></th>

			</tr>
		</thead>
		<tbody>
		    <c:if test="${not empty invStatistics}">
                <input type="hidden" name="sListSize" id="lSize" value='${invStatistics.size()}'>
                <c:forEach items="${invStatistics}" var="curObject">
                    <tr>
                        <td align="left">${curObject.inventoryId}</td>
                        <td align="left">${curObject.inventoryCode}</td>
                        <td align="left">${curObject.invName}</td>
                        <td align="left">${curObject.location}</td>
                    </tr>
                </c:forEach>
            </c:if>
		</tbody>
...
function search()
{
	document.forms[0].action = "Item?tabid=InvStatistic";
