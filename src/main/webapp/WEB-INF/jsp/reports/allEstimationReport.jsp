...
<title>${sessionScope.user} - <spring:message code="BzComposer.Report.AllEstimation"/></title>
<style>
...
<body>
<form:form action="EstimationBoard?tabid=AllEstimationList" method="post" modelAttribute="estimationBoardDto">
...
<div id="printContent">	
	<div id="headerBar">
	      ...
	      <h6 style="text-align: center;color: blue;" id="headerBarValue"><spring:message code="BzComposer.Report.AllEstimation"/></h6>
	</div>
	<div id="table-negotiations" style="overflow:auto;height:500; text-align: center;">
		...
		<thead>
			<tr>
				<th><spring:message code="BzComposer.Estimaion.EstNum" /></th>
				<th><spring:message code="BzComposer.accounting.Date" /></th>
				<th><spring:message code="BzComposer.Report.AllInvoice.ClientVendor" /></th>
				<th><spring:message code="BzComposer.Invoice.Rep" /></th>
				<th><spring:message code="BzComposer.sales.SalesAmount" /></th>
...
<%-- 		<input type="hidden" name="sListSize" id="lSize" value='<bean:write name="catList" />'> --%>
            <c:forEach items="${EstimationBoardDetails}" var="curObject">
                <tr>
                <td align="left">${curObject.est_no}</td>
                <td align="left">${curObject.dateAdded}</td>
                <td align="left">${curObject.lastName},${curObject.firstName}</td>
                <td align="left">${curObject.rep}</td>
                <td align="left">${curObject.total}</td>
                <td></td>
                </tr>
            </c:forEach>
            </c:if>
...
function search()
{
	document.forms[0].action = "EstimationBoard?tabid=AllEstimationList";
