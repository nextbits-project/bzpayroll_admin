...
<title>${sessionScope.user} - BzComposer - Refund Invoice Report</title>
<style>
table.tabla-customListOds { width: 70%; border: 1px solid rgb(207, 207, 207); margin:auto; }
...
<body>
<form:form action="SalesBord.do?tabid=SalesRID&ilist=3" method="post" modelAttribute="salesBoardDto">
	...
	<div class="report-form-underheader" style="width: auto;">
		<table>
			<tr>
	    		...
	    		<td>
	    			<label style="padding-left: 15px"><spring:message code="BzComposer.sales.FromDate"/></label>
    			</td>
	    		<td>
	    		...
	    		<td>
	    			...
	    			onclick="displayCalendar(document.salesboardForm.fromDate,'mm-dd-yyyy',this);" style="padding-left: 5px">
    			</td>
	    		<td>
	    			<label style="padding-left: 15px"><spring:message code="BzComposer.sales.to"/></label>
    			</td>
	    		<td>
	    		...
	    		<td>
	    			<img src="images/cal.gif"
	    			onclick="displayCalendar(document.salesboardForm.toDate,'mm-dd-yyyy',this);" style="padding-left: 5px">
    			</td>
	    		<td>
	    			<label style="padding-right: 10px;padding-left: 15px"><spring:message code="BzComposer.Report.labelSortBy"/></label>
    			</td>
	    		<td>
	    	    ...
	    	    <td>
	    	    	<input type="button" value='<spring:message code="BzComposer.sales.Search"/>' class="formbutton mar" style="margin-left: 50px" onclick="search()">
    	    	</td>
	    	</tr>
		</table>
	</div>
	<div id="printContent">	
		<div id="headerBar">
			...
			<h6 style="text-align: center;color: blue;" id="headerBarValue">Refund Invoice Report</h6>
		</div>
		<div id="table-negotiations" style="overflow:auto;height:500; text-align: center;">
			<table class="tabla-customListOds" cellspacing="0" id="exportPd" border="1">
				<thead>
					<tr>
						<th><spring:message code="BzComposer.Invoice.InvoiceNo"/></th>
						<th><spring:message code="BzComposer.Invoice.Date"/></th>
						<th><spring:message code="BzComposer.Report.labelSortBy.client/vendor"/></th>
						<th><spring:message code="Bzcomposer.Report.refundedBy"/></th>
						<th><spring:message code="BzComposer.Report.labelSortBy.amount"/></th>
					</tr>
				</thead>
				<tbody>
					<c:if test="${not empty refundInvoice}">
					<%-- <input type="hidden" name="sListSize" id="lSize" value='${catList}'> --%>
						<c:forEach items="${refundInvoice}" var="sale">
							<tr>
...
function search()
{
	document.forms[0].action = "SalesBord?tabid=refundInvoiceReport";
