...
<title>${sessionScope.user} - <spring:message code="BzComposer.Report.Missing.InventoryList"/></title>
<style>
...
<body>
<form:form action="Item?tabid=MissingInventoryList" method="post" modelAttribute="itemDto">
	...
	<span style="font-size: 1.1em; font-weight: normal; color: #838383; margin: 30px 0px 15px 0px; border-bottom: 1px dotted #333; padding: 0 0 .3em 0;">
		<spring:message code="BzComposer.Report.Missing.InventoryList" />
		</span></div>
	<div>
	...
	<div id="headerBar">
          ...
          <h6 style="text-align: center;color: blue;" id="headerBarValue"><spring:message code="BzComposer.Report.Missing.InventoryList"/></h6>
    </div>
	<table align="center">
		<tr>
	...
	<table class="tabla-listados" cellspacing="0">
		<thead>
			<tr>
				<th><spring:message code="BzComposer.Report.Missing.InventoryList.InventoryName" /></th>
				<th><spring:message code="BzComposer.Report.Missing.InventoryList.Description" /></th>
				<th><spring:message code="BzComposer.Report.Missing.InventoryList.AdjustedQty" /></th>
				<th><spring:message code="BzComposer.Report.Missing.InventoryList.Date" /></th>
				<th><spring:message code="BzComposer.Report.Missing.InventoryList.Reason" /></th>
				<th><spring:message code="BzComposer.Report.Missing.InventoryList.Ref.CustomerRMA" /></th>
			</tr>
		</thead>
		<tbody>
			<c:if test="${not empty missingInventoryList}">
				<%-- 	<input type="hidden" name="sListSize" id="lSize" value='${missingInventoryList.size()}'> --%>
                <c:forEach items="${missingInventoryList}" var="curObject">
                    <tr >
                        <td align="left">${curObject.inventoryName}</td>
                        <td align="left">${curObject.inventoryDescription}</td>
                        <td align="left">${curObject.Adjustqty}</td>
...
function callRefresh(){
	document.forms[0].action = "Item?tabid=MissingInventoryList";
