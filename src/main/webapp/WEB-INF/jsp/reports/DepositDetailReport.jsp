...
<title>${sessionScope.user}-<spring:message code="BzComposer.Report.Banking&Accounting.DepositDetail"/></title>
<style>
...
<body>
<form:form action="BankingAccounting?tabid=DepositDetail" method="post" modelAttribute="accountDto">
...
<div id="printContent">
	...
	<div id="headerBar">
        ...
        <h6 style="text-align: center; color: blue;"
            id="headerBarValue">
            <spring:message code="BzComposer.Report.Banking&Accounting.DepositDetail" />
        </h6>
    </div>
	...
	<table class="tabla-customListOds" id="exportPd" border="1">
		<thead>
			<tr>
					...
					<th ><spring:message code="BzComposer.ClientId" /></th>
					<th ><spring:message code="BzComposer.Report.Amount" /></th>
			</tr>
		</thead>
		<tbody>
			<c:if test="${not empty depositDetail}">
                <input type="hidden" name="sListSize" id="lSize" value='${depositDetail.size()}'>
                <c:forEach items="${depositDetail}" var="curObject">
                    <tr>
                        <td>${curObject.bankName}</td>
                        <td>${curObject.date}</td>
                        <td>${curObject.invoiceId}</td>
                        <td>${curObject.clientVendorId}</td>
...
function search()
{
	document.forms[0].action = "BankingAccounting?tabid=DepositDetail";
