...
<title>${sessionScope.user} - <spring:message code="BzComposer.SalesPanel.SalesByItem"/></title>
<style>
...
<div id="printContent">
	<div id="headerBar">
	      ...
	      <h6 style="text-align: center;color: blue;" id="headerBarValue"><spring:message code="BzComposer.SalesPanel.SalesByItem"/></h6>
	</div>
...
<%-- 		<input type="hidden" name="sListSize" id="lSize" value='${catList}'> --%>
            <c:forEach items="${salesRBC}" var="sale">
                <tr>
                <td>${sale.cvName}</td><td></td>
                <td>${sale.inventoryQty}</td>
                <td>${sale.amount}</td>
                <td>${sale.balTotal}</td>
                </tr>
			</c:forEach>
