...
<title>${sessionScope.user}-<spring:message code="BzComposer.Report.Banking&Accounting.AccountPayable"/></title>
<style>
...
<body>
	<form:form action="Customer?tabid=AccountPayable" method="post" modelAttribute="customerDto">
	...
	<div id="headerBar">
        ...
        <h6 style="text-align: center; color: blue;"
            id="headerBarValue">
            <spring:message code="BzComposer.Report.Banking&Accounting.AccountPayable" />
        </h6>
    </div>
                ...
                style="overflow: auto; height: 400; text-align: center;">
                    <table class="tabla-customListOds" id="exportPd" border="1">
                        ...
                        <thead>
                            <tr>
                                <th><spring:message code="BzComposer.Report.Rcolumn.Ref" /></th>
                        ...
                        <tbody>
                            <c:if test="${not empty acPayList}">
                                <input type="hidden" name="sListSize" id="lSize" value='${acPayList.size()}'>
                                <c:forEach items="${acPayList}" var="curObject">
                                    <tr>
                                        <td align="left">${curObject.poNum}</td>
                                        <td align="left">${curObject.dateAdded}</td>
                                        <td align="left">${curObject.fullName}</td>
                                        <td align="left">${curObject.total}</td>
                                        <td align="left">${curObject.balance}</td>
                                    </tr>
                                </c:forEach>
                            </c:if>
                        </tbody>
...
function search()
{
	document.forms[0].action = "Customer?tabid=AccountPayable";
