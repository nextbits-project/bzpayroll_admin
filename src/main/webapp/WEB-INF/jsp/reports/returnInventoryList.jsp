...
<title>${sessionScope.user} - <spring:message code="BzComposer.Report.Return.InventoryList"/></title>
<style>
...
<body>
<form:form action="Item?tabid=ReturnInventoryList" method="post" modelAttribute="itemDto">
	...
	<div id="headerBar">
          ...
          <h6 style="text-align: center;color: blue;" id="headerBarValue"><spring:message code="BzComposer.Report.Return.InventoryList"/></h6>
    </div>
	<table align="center">
		<tr>
	...
	<table class="tabla-listados" cellspacing="0">
		...
		<thead>
			<tr>
				<th><spring:message code="BzComposer.Report.Return.InventoryList.InventoryName" /></th>
				<th><spring:message code="BzComposer.Report.Return.InventoryList.InventoryId" /></th>
				<th><spring:message code="BzComposer.Report.Return.InventoryList.RmaItemQty" /></th>
				<th><spring:message code="BzComposer.Report.Return.InventoryList.Reason" /></th>
				<th><spring:message code="BzComposer.Report.Return.InventoryList.AdjustedQty" /></th>
				<th><spring:message code="BzComposer.Report.Return.InventoryList.Date" /></th>
				<th><spring:message code="BzComposer.Report.Return.InventoryList.RmaId" /></th>
				<th><spring:message code="BzComposer.Report.Return.InventoryList.RmaNo" /></th>
			</tr>
		</thead>
		<tbody>
			<c:if test="${not empty missingInventoryList}">
<%--     	<input type="hidden" name="sListSize" id="lSize" value='${missingInventoryList}'> --%>
                <c:forEach items="${missingInventoryList}" var="curObject">
                    <tr>
                        <td align="left">${curObject.inventoryName}</td>
                        <td align="left">${curObject.inventoryID}</td>
                        <td align="left">${curObject.rmaitemqty}</td>
                        <td align="left">${curObject.Reason}</td>
                        <td align="left">${curObject.TotalAdjustedqty}</td>
                        <td align="left">${curObject.DateAdded}</td>
                        <td align="left">${curObject.RmaID}</td>
                        <td align="left">${curObject.RefCustomerRMAno}</td>
                    </tr>
                </c:forEach>
			</c:if>
		</tbody>
...
function search()
{
	document.forms[0].action = "Item?tabid=ReturnInventoryList";
