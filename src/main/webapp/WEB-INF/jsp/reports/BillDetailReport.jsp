...
<title>${sessionScope.user}-<spring:message code="BzComposer.Report.Banking&Accounting.BillDetail"/></title>
<style>
...
<body>
<form:form action="BankingAccounting?tabid=BillDetail" method="post" modelAttribute="accountDto">
...
<div id="printContent">
	...
	<div id="headerBar">
        ...
        <h6 style="text-align: center; color: blue;"
            id="headerBarValue">
            <spring:message code="BzComposer.Report.Banking&Accounting.BillDetail" />
        </h6>
    </div>
	...
	<table class="tabla-customListOds" id="exportPd" border="1">
		<thead>
			<tr>
					<th ><spring:message code="BzComposer.Report.Banking&Accounting.BillNumber" /></th>
					<th><spring:message code="BzComposer.vendor.vendorsName" /></th>
					<th ><spring:message code="BzComposer.Invoice.Date.DateAdded" /></th>
					<th ><spring:message code="BizComposer.Configuration.Reminders.DueDate" /></th>
					<th ><spring:message code="BzComposer.Employee.Memo" /></th>
					<th ><spring:message code="BzComposer.Report.Lists.PaymentStatus" /></th>
					<th ><spring:message code="BzComposer.accounting.CheckNo" /></th>
					<th ><spring:message code="BzComposer.Report.Amount" /></th>
			</tr>
		</thead>
		<tbody>
			<c:if test="${not empty billDetail}">
                <input type="hidden" name="sListSize" id="lSize" value='${billDetail.size()}'>
                <c:forEach items="${billDetail}" var="curObject">
                    <tr>
                        <td>${curObject.billNum}</td>
                        <td>${curObject.vendorName}</td>
                        <td>${curObject.date}</td>
                        <td>${curObject.dueDate}</td>
                        <td>${curObject.memo}</td>
                        <td>${curObject.paymentStatus}</td>
                        <td>${curObject.chekcno}</td>
...
function search()
{
	document.forms[0].action = "BankingAccounting?tabid=BillDetail";
