...
<title>${sessionScope.user} - <spring:message code="BzComposer.Report.Profit&Budget.ProfitLossStnd"/></title>
<style>
...
<body>
<form:form action="Category?tabid=ProfitLoss" method="post">
...
<div id="printContent">	
	<div id="headerBar">
	      ...
	      <h6 style="text-align: center;color: blue;" id="headerBarValue"><spring:message code="BzComposer.Report.Profit&Budget.ProfitLossStnd"/></h6>
	</div>
	<div id="table-negotiations" style="overflow:auto;height:500; text-align: center;">
		...
		<tbody>
			<c:if test="${not empty profitLoss}">
<%-- 		    <input type="hidden" name="sListSize" id="lSize" value='${profitLoss.size()}'> --%>
 			    <c:forEach items="${profitLoss}" var="curObject">
                    <tr>
...
function search()
{
	document.forms[0].action = "Category?tabid=ProfitLoss";
