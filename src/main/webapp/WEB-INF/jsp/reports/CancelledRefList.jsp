...
<title><spring:message code="BzComposer.Report.CancelledPurchase"/></title>
<style>
...
<body>
<form:form action="PurchaseBoard?tabid=CancelledPurREfBill" method="post" modelAttribute="purchaseBoardDto">
...
<div id="printContent">
	<div class="report-form-underheader">
	 <table>
	    	<tr>
	    		...
	    		<td><img src="images/cal.gif" onclick="displayCalendar(document.PurchaseBoardForm.fromDate,'mm-dd-yyyy',this);" style="padding-left: 5px"></td>
	    		<td><label style="padding-left: 15px"><spring:message code="BzComposer.sales.to"/></label></td>
	    		<td><form:input path="toDate" size="15"/></td>
	    		<td><img src="images/cal.gif" onclick="displayCalendar(document.PurchaseBoardForm.toDate,'mm-dd-yyyy',this);" style="padding-left: 5px"></td>
	...
	<div id="headerBar">
        ...
        <h6 style="text-align: center; color: blue;"
            id="headerBarValue">
            <spring:message code="BzComposer.Report.CancelledPurchase" />
        </h6>
    </div>
	...
	<table class="tabla-customListOds" id="exportPd" border="1">
		<thead>
			<tr>
					<th ><spring:message code="BzComposer.purchase.RefPONumPrefix" /></th>
					<th><spring:message code="BzComposer.Report.ReservedInventoryList.Date" /></th>
					<th ><spring:message code="BzComposer.Valuation.InventoryValuation.Detail.Customer" /></th>
					<th ><spring:message code="BzComposer.Vendor.Rep" /></th>
					<th ><spring:message code="BzComposer.Invoice.Amt" /></th>
					<th ><spring:message code="BzComposer.accounting.PaymentAmount" /></th>
					<th ><spring:message code="BzComposer.accounting.Balance" /></th>
					</tr>
		</thead>
		<tbody>
		    <c:if test="${not empty cancelledPuBillRef}">
                <input type="hidden" name="sListSize" id="lSize" value='${cancelledPuBillRef.size()}'>
                <c:forEach items="${cancelledPuBillRef}" var="curObject">
                    <tr>
                        <td>${curObject.po_no}</td>
                        <td>${curObject.dateAdded}</td>
                        <td>${curObject.cvName}</td>
                        <td>${curObject.repname}</td>
                        <td>${curObject.total}</td>
                        <td>${curObject.paidAmount}</td>
                        <td>${curObject.balance}</td>
                    </tr>
                </c:forEach>
            </c:if>
		</tbody>
...
function search()
{
	document.forms[0].action = "PurchaseBoard?tabid=CancelledPurREfBill";
