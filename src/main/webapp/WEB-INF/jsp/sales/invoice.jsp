...
$(function()
	...
	if(checked == true){
		...
		$.ajax({
			type: "POST",
			url:"CustomerAjax?tabid=SortInvoice&SortBy=3",
			data:{sortBy : "2"},
			success : function(data) {
				...
				select.setAttribute("onchange", "Assignment(this.value, this.form);");

				// Static Option 1: Select
				var defaultOpt = document.createElement("option");
				defaultOpt.value = "0";
				defaultOpt.textContent = "Select";
				select.appendChild(defaultOpt);

				// Static Option 2: Add New Customer
				...
				for (var i = 0; i < data.length; i++) {
					...
					option.value = data[i].clientVendorID;
					option.textContent = data[i].companyName +"("+data[i].lastName +", "+data[i].firstName+")";
					select.appendChild(option);
				}

				// Replace the dropdown
				document.getElementById("custDiv").innerHTML = "";
				document.getElementById("custDiv").appendChild(select);
			},
			error : function(data) {
	...
	} else
	{
		$.ajax({
			type: "POST",
			url:"CustomerAjax?tabid=SortInvoice&SortBy=1",
			data:{sortBy : "1"},
			success : function(data) {
				var select = document.createElement("select");
				select.id = "custID";
				...
				<div id="blanquito">
					<div id="padding">
						<form:form name="InvoiceForm" id="frmInvoice" method="post"
											...
											<tr>
												...
												<td>
													<div style="padding-top: 38px;text-align: center;">
														<c:if test="${invoiceDto.invoiceStatus == 1}">
															<span style=" font-size: 30px; color: red !important; ">Canceled</span>
														</c:if>
														<c:if test="${invoiceDto.invoiceStatus == 2}">
													...
													<table>
														<tr>
															<td id="ship_id"
																...
																<form:textarea path="shipTo" id="shipTo" rows="6" cols="25"
													...
													<table align="left">
														...
														<tr>
															...
															<td id="via_id" style="font-size: 14px;"><form:select
																	id="via" path="via" style="width:220px"
											...
											<td style="font-size: 14px;" colspan="3">
												<table align="right">
													...
													<tr>
														...
														<td align="right" style="font-size: 14px;"><spring:message
																code="BzComposer.Invoice.ShippingCharges" /></td>
														<td style="font-size: 14px;"><form:input
								...
								<div class="row">
									<div class="col-md-12" style="font-size: 16px;" align="center">
										...
										<input type="button" id="btnSaveInvoice" class="formbutton"
											onclick="confirmAddressChange(this.form);" title="Save Invoice"
							...
							<input type="hidden" id="invoiceStatus" value="${invoiceDto.invoiceStatus}"/>
						</form:form>
					</div>
				</div>
			</div>
        ...
        buttons: {
            "<spring:message code='BzComposer.global.ok'/>": function () {
				...
				$.ajax({
					...
					data: {type:"invoice", orderNo: orderNo},
					success: function (data) {
...
function SendMail(form) {
    ...
    const url = "Invoice?tabid=ShowEmail&OrderType=invoice&OrderNo=" + orderNo;
...
function printCustomerOrder(form){
	...
	let orderNo = form.orderNo.value;
    if(cid==0){
        return showValidationDialog();
    } else {
        event.preventDefault();
            ...
            buttons: {
                "<spring:message code='BzComposer.global.print'/>": function () {
                    ...
                    if (document.getElementById("PrintInvoiceCbx").checked) {
						window.open(
							"Invoice?tabid=PrintInvoice&custID=" + encodeURIComponent(cid) + "&ttype=" + encodeURIComponent(ttype) + "&orderNo=" + encodeURIComponent(orderNo),
							"_blank",
						...
						window.open(
							"Invoice?tabid=PrintPackingSlip&custID=" + encodeURIComponent(cid) + "&orderNo=" + encodeURIComponent(orderNo),
							"_blank",
							"scrollbars=yes,height=900,width=900,status=yes,toolbar=no,menubar=no,location=no"
						);
					}
