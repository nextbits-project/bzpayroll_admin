...
$(document).ready(function() {
 ...
 customerTable = $('#custTable').DataTable({
    ...
    columns: [
      ...
      { data: "cname", title: "Name" }
    ],

   autoWidth: false,   // Prevent DataTables auto-stretch
  ...
  console.log("[DEBUG] sortBY ",sortBy);
	$.ajax({
        ...
        for(var i=0; i<data.length; i++){
            var objList = data[i];
                                  console.log("objListtttttt : "  , objList);
            custDetails = custDetails + "<tr id='"+i+"$$' onclick='setCutomerDataById("+objList.clientVendorID+", "+i+")'>"
                +"<td colspan='2' style='font-size:12px;'>"+objList.clientVendorID+" : "+objList.companyName+" ("+objList.firstName+" "+objList.lastName+")</td></tr>";
        }
...
<script>
  ...
  <c:forEach items="${CustomerDetails}" var="c">
    console.log("ID: ${c.clientVendorID}, Name: ${c.cname}, Phone: ${c.active}");
  </c:forEach>
</script>
<style type="text/css">
      ...
      <div class="col-md-12">
        <nav>
          <div class="nav nav-tabs nav-fill" id="nav-tab" role="tablist">
            <a class="nav-item nav-link active" id="nav-home-tab" href="#nav-home" role="tab">Active</a>
            <a class="nav-item nav-link" id="nav-profile-tab" href="#nav-profile" role="tab">Inactive</a>
          </div>
        </nav>
        ...
        <div class="tab-content" id="nav-tabContent" style="background-color: white">
          ...
          <div class="tab-pane fade" id="nav-profile" role="tabpanel">
            <p class="p-3">Inactive customers will be listed here…</p>
          </div>
...
document.querySelectorAll('#nav-tab a').forEach(tab => {
  tab.addEventListener('click', function (e) {
    ...
    document.querySelectorAll('#nav-tab a').forEach(t => t.classList.remove('active'));
...
$(document).ready(function () {
  $('#custTable tbody').on('click', 'tr', function () {
    ...
    $('#custTable tbody tr').removeClass('selected-row');
