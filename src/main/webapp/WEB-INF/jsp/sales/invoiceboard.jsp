...
<body onload="Init();">
	...
	<form:form id="invoiceForm" name="salesboardForm"
														...
														<tbody>
															<c:forEach items="${SalesBoardDetails}" var="objList"
																...
																<tr id='${loop.index}$$' ondblclick="sendToInvoice()"
																	onclick="setRowId(${objList.orderNum}, '${objList.email}', ${loop.index}, true, ${objList.paymentCompleted == true ? 1 : 0}, ${objList.shipped})">
																	...
																	<td style="font-size: 14px;">
																		<c:if test="${objList.paymentCompleted == true}">
																			<input type="checkbox" name="paymentCompletedCHK"
																				id="paymentCompletedId" onclick="isPaymentCompleted('${objList.invoiceID}')" title="paymentCompleted"
																				checked="true">
																		</c:if> 
																	...
																	<td style="font-size: 14px;">
																		<c:if test="${objList.printed == true}">
																			<input type="checkbox" onclick="isPrinted('${objList.invoiceID}')" name="isPrintedCHK"
																	...
																	<td style="font-size: 14px;">
																		<c:if test="${objList.shipped == '1'}">
																			<input type="checkbox" name="shipped${loop.index}"
																				id="shippedId" onclick="isShipped('${objList.invoiceID}')" title="shipped" checked="true"
																				value="ON" />
																		</c:if> 
																		<c:if test="${objList.shipped == '0'}">
																			<input type="checkbox"
																				name="shippedshipped${loop.index}" id="shippedId"
																				title="shipped" onclick="isShipped('${objList.invoiceID}')" value="ON" />
																		</c:if>
																	</td>
																	<td><c:if test="${objList.emailed == '1'}">
																			<input type="checkbox" value="ON"
																				name="isEmailValCHK" id="isEmailId"
																				onclick="isEmailClick('${objList.invoiceID}')"
		...
		<input type="hidden" id="is_payment_completed" />
		<input type="hidden" id="is_shipped" />
...
function setRowId(rowId, email, rowIndex, flag, isPaymentCompleted, isShipped){
	document.getElementById("is_payment_completed").value=isPaymentCompleted;
	document.getElementById("is_shipped").value=isShipped;
...
function sendToCreateRMA(){
	isPaymentCompleted = document.getElementById("is_payment_completed").value;
	isShipped = document.getElementById("is_shipped").value;
	if(isPaymentCompleted == '1' && isShipped == '1'){
    	order_no = document.getElementById("ord_value").value;
    	window.location = "RMA?tabid=CreateRMA&orderNumber="+order_no;
	}else{
		paymentNotCompletedDialog();
	}
}
...
function paymentNotCompletedDialog(){
	$("#paymentNotCompletedDialog").dialog({
		resizable: false,
		height: 200,
		width: 500,
		modal: true,
...
<div id="paymentNotCompletedDialog" style="display: none;">
	<p><spring:message code="BzComposer.payment_not_completed" /></p>
</div>
