...
$(function() {
    ...
    $('#servicedrop_itemSubCategory').prop('disabled', 'disabled');
 ...
 $(function(){
   ...
   $('#addCrossSell').click(function(){
	...
	$('#servicesubItem').change(function()
			{
		if($(this).prop("checked") == true)
		{
			
			$('#servicedrop_itemSubCategory').prop('disabled', false);
		}
		else
		{
			$('#drop_cat').prop('disabled', 'disabled');
...
<form:form action="Item" method="post" enctype="MULTIPART/FORM-DATA" id="AddItemfrm2" name="AddItemfrm" modelAttribute="itemDto">
<div id="cos">
				...
				<td colspan="3" align="center" style="padding-right: 14px;">
					...
					<div id="serviceTab" style="display:none;">
						...
						<table class="tabla-listados" cellspacing="0" style="margin-bottom:250px;">
								...
								<tr>
									<td><input type="checkbox" id="servicesubItem" name="servicesubItem" value="servicesubItem" >
												<spring:message code="BzComposer.categorymanager.subcategory"/></td>
									
									<td>			
													<span><spring:message code="BzComposer.CompulsoryField.Validation" /></span>	
												<form:select path="serviceitemSubCategory" id="servicedrop_itemSubCategory"  >
								...
								<tr>
									...
									<td>
										<form:input type="text" path="serviceItemTitle"/>
									</td>
									<td colspan="4">&nbsp;</td>
								</tr>
								<tr>
									...
									<td>
										<form:input type="text" path="serviceItemRate"/>
									</td>
									<td>
									...
									<td>
										<select id="unitTime">
											...
											<option value="7">semi-yearly</option>
								...
								<tr>
									...
									<td colspan="2">
										...
										<form:textarea path="serviceItemDescription"  rows="5" cols="45"/>
									</td>
									<td colspan="3">&nbsp;</td>
								</tr>
								</tbody>
						...
						<div id="table-negotiations">
							<table class="tabla-listados" cellspacing="0" style="margin-bottom:250px;">
								...
								<tr>
									...
									<td width="15%">
										<form:input type="text" path="recurringServiceCode" />
									</td>
								...
								<tr>
									...
									<td width="10%">
										<form:select path="unitTime" style="width:150px;">
											...
											<option value="6">semi-yearly</option>
								...
								<tr>
									...
									<td colspan="2">
										<form:textarea rows="5" cols="45" path="serviceDescription"></form:textarea>
									</td>
									<td colspan="3">&nbsp;</td>
								</tr>
								</tbody>
...
function CheckMambership(form) {
	...
	if((membershipLevel.toLowerCase()===st.toLowerCase())||(membershipLevel.toLowerCase()===sl.toLowerCase()))
	{
		if(size>=100){
	...
	else if(membershipLevel.toLowerCase() == pf.toLowerCase() || membershipLevel.toLowerCase() == gd.toLowerCase()){
		if(size>=500){
    ...
    files.forEach((file, index) => {
        if (file.type.startsWith("image/")) {
            ...
            reader.onload = function (e) {
				...
				let imgHTML = '<div class="col-md-4 image-container position-relative" id="image-'+uniqueId+'" style="padding: 5px;">' +
...
function ShowAdd(form){
				...
				$("#showItemDialog").dialog({
                    ...
                    buttons: {
                        "<spring:message code='BzComposer.global.ok'/>": function () {
							...
							let formElement = document.forms['AddItemfrm'];
							let formData = new FormData(formElement);
							uploadedFiles.forEach((file, index) => {
								formData.append('images', file);
							});

							formData.append('ItemType', val);

							fetch("Item?tabid=AddItem", {
								method: "POST",
								body: formData
							})
							.then(response => {
	...
	else if(val=="2")
	{
		...
		if( document.AddItemfrm.serviceItemCode.value=="")
		{
			return  fillAllFields();
		}
          if(document.AddItemfrm.serviceItemRate.value=="")
        	  {
        	  document.AddItemfrm.serviceItemRate.focus();
        	  return  fillAllFields();
        	  
        	  }
          if(document.AddItemfrm.serviceItemTitle.value=="")
    	  {
        	  document.AddItemfrm.serviceItemRate.focus();
        	  return fillAllFields();
        	  
    	  }
          if(document.AddItemfrm.serviceItemDescription.value=="")
    	  {
        	  document.AddItemfrm.serviceItemRate.focus();
        	  return fillAllFields();
		...
		if(form.recurringServiceCode.value==""){
			return showitemcodesubdialog();
			form.serviceCode.focus();
		}
		else{
			event.preventDefault();
			$("#showItemDialog").dialog({
...
function loadServiceSubCategoryByCatID(){

   
        $.ajax({
            type: "GET",
            url:"/ItemSubCategoryList?tabid=loadSubCategoryByCatID&id="+11,
            success:function(data){
            	
            	   var subCatDropdown = document.getElementById('servicedrop_itemSubCategory');
               
                for(var i=0;i<data.length;i++)
                {
                	 var  option = document.createElement( 'option' );
                     option.value =data[i].subItemCategoryID;
...
<div id="fillAllFields" style="display:none;">
	<p><spring:message code="BzComposer.service.serviceitemdata"/></p>
</div>
...
<script>
	...
	// 		const input = $(`#${selectedSupplierId}`).find('input');
	// 		const newName = prompt("Edit supplier name:", input.val());
	// 		if (newName !== null) {
	// 			input.val(newName);
	// 		}
	// 	} else {
