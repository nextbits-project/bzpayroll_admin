...
<form:form method="post" id="frmSendMail" modelAttribute="emailSenderDto">
	...
	<table cellpadding="0" cellspacing="0" border="0" align=center style="width: 100%;">
        <c:if test="${not empty Result}">
            <tr>
                <td colspan="3">
                    <span class="msgstyle">${Result}</span>
                </td>
            </tr>
        </c:if>
    </table>
	...
	<!-- end Contents -->
	<input type="hidden" id="custIDs" value="${custIDs}" />
</form:form>
...
debugger;
if (resultMsg === "Mail send Successfully") {
   window.close();
}
function CloseMe(){
	window.close();
...
function Send(emailaddr){
	if(document.getElementById('from').value.trim()=="" || document.getElementById('to').value.trim()==""){
		...
		document.forms["frmSendMail"].action = "Invoice?tabid=SendMail&custIDs="+document.getElementById('custIDs').value;
