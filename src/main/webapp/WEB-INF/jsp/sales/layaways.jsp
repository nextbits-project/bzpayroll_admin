								...
								<div class="row">
									<div class="col-md-12" style="font-size: 16px;" align="center">
										...
										<input type="button" id="btnSaveSalesOrder" class="formbutton"
											onclick="confirmAddressChange(this.form);" title="Save Sales Order"
...
function clearItemData() {
    ...
    document.getElementById('pname_id').value = "";
    document.getElementById('itemID').value = "0";
    document.getElementById('amt_id').value = "0.00";

    // Reset totals
    document.getElementById('subtotal').value = "0.00";
    document.getElementById('total').value = "0.00";
    document.getElementById('adjustedtotal').value = "0.00";
    document.getElementById('tax').value = "0.00";
    document.getElementById('weight').value = "0.00";
    document.getElementById('wt').value = "0.00";

    // Clear dynamically added rows
    const table = document.getElementById('tr##').parentNode;
...
function cancelInvoice(){
	...
	event.preventDefault();
 	$("#cancelInvoice").dialog({
        ...
        buttons: {
            "<spring:message code='BzComposer.global.ok'/>": function () {
				var orderNo = document.getElementById("salesorder_no").value;

				$.ajax({
					type: "POST",
					url: "SalesAjax?tabid=CancelInvoice",
					data: {type:"layaway", orderNo: orderNo},
					success: function (data) {
...
function confirmAddressChange(form) {
	...
	function transformToInvoice(form){
		...
		function SendMail(form){
			...
			window.open("Invoice?tabid=ShowEmail&OrderType=Layaway&OrderNo="+cid,null,"scrollbars=yes,height=500,width=900,status=yes,toolbar=no,menubar=no,location=no" );
		}
