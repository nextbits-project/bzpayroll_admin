...
#custTable th:first-child,
#custTable td:first-child {
  width: 10%;   /* fixed size instead of percentage */
...
function fetchCustomers(url, data = {}) {
  ...
  $.ajax({
    ...
    success: function(responseData) {
      console.log("[DEBUG] Data received1 : ", responseData);

      //passing responseData to phoneCall Tab
      customerTable.clear(); // remove loading

      if (Array.isArray(responseData) && responseData.length > 0) {
        
        console.log("[DEBUG] responseData 2", responseData);
        customerTable.rows.add(responseData).draw(false);
        showCustomerInfo(responseData[0]);
  ...
  // Helper to show data in customer-info box
function showCustomerInfo(customer) {
  console.log("[DEBUG] CUSTOMER " , customer);
  $('#customerFullName').text(customer.fullName || customer.cname || "N/A");
  $('#companyName').text(customer.cname || customer.companyName || "N/A");
}

// Clear info if no records
...
<!-- Table -->
 <table id="custTable" class="display" style="">
    <thead  style="">
      <tr>
