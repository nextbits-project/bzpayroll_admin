...
<title>BzPayroll - Company Information List</title>
<style>
.dataTables_length {
	font-size: 14px;
}
...
#tabs .tabla-listados thead th a:hover {
  color: #0056B3 !important;      /* on hover → red */
</style>
</head>
<script>
$(function() {
...
<body onload="initialize();">
 ...
 <!-- Status Tab -->
      <div id="statusTab">--%>
						...
						<input type="hidden" id="lSize" value='${companyList.size()}' />
							...
							<tbody id="custTableBody">
                                <c:if test="${not empty companyList}">
                                    <c:forEach items="${companyList}" var="company" varStatus="loop">
                                        <tr id='${loop.index}$$'
                                            onclick="setRowId(${company.id}, ${loop.index}, true);">

                                            <!-- Checkbox and ID -->
                                            <td>
                                                <input type="checkbox" id="companyID${loop.index}" value="${company.id}"
                                                       onchange="addCompanyRowIndex(${loop.index}, ${company.id})" />
                                                <a class="td-id" href="#">${company.id}</a>
                                            </td>

                                            <!-- Company Info -->
                                            <td>${company.membershipLevel}</td>
                                            <td>${company.emailAddress}</td>
                                            <td>${company.firstName}</td>
                                            <td>${company.lastName}</td>
                                            <td>${company.jobPosition}</td>
                                            ...
                                            <td>
                                                <c:choose>
                                                    <c:when test="${company.sameAsPhone}">Yes</c:when>
                                                    <c:otherwise>No</c:otherwise>
                                                </c:choose>
                                            </td>
                                            <td>${company.fax}</td>
                                            <td>${company.employerId}</td>
                                        </tr>
                                    </c:forEach>
                                </c:if>
                            </tbody>
