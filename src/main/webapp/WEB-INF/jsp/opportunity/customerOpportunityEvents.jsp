     ...
     // week: 'Week',
      ...
      eventClick: function(arg,info)
      {
        ...
        if (confirm('Are you sure you want to edit  this event?')) 
        {
        	 ...
        	 document.getElementById("event_opportunityID").value=arg.event.extendedProps.opportunityID;
	   ...
	   allevents.push({
           ...
           title:document.getElementById(i+"eventName").value,
            ...
            extendedProps: {
            	 ...
            	 opportunityID:document.getElementById(i+"eventOpportunityID").value
              },
								...
								<div class="col-sm-12">
									...
									<div class="form-group">
									 <label for="event_id"> <spring:message code="BzComposer.customer.opportunity.eventId" />
										...
										<label
										class="required"	for="opportunity_name"> <spring:message
												code="BzComposer.customer.opportunityName" />
										</label> 
										<select name="opportunityID" id="event_opportunityID" style="width:200px;" class="form-control">
                                                            <option value="0"><spring:message code="BzComposer.ComboBox.Select" /></option>
                                                            <c:forEach items="${opportunities}" var="item">
                                                                <option value="${item.opportunityId}">${item.name}</option>
                                                            </c:forEach>
                                        </select>
							...
							<div class="row">
								...
								<div class="col-sm-12">
									...
									<div class="form-group">
										...
										<input type="hidden" name="addEvent_id" id="addEvent_id"> 
										
										<label
										class="required"	for="opportunity_name"> <spring:message
												code="BzComposer.customer.opportunityName" />
										</label> 
										<select name="opportunityID" id="addopportunityID" style="width:200px;" class="form-control">
                                                            <option value="0"><spring:message code="BzComposer.ComboBox.Select" /></option>
                                                            <c:forEach items="${opportunities}" var="item">
                                                                <option value="${item.opportunityId}">${item.name}</option>
                                                            </c:forEach>
				...
				<c:forEach items="${eventList}" var="event" varStatus="loop">
					...
					<input type="hidden" id='${loop.index}eventEndDate'
						value='${event.eventEndDate}' />
							<input type="hidden" id='${loop.index}eventOpportunityID'
...
<script>
   ...
   var event_name = $("#addEvent_name").val();
   		...
   		var opportunityID=$("#addopportunityID").val();
   		
   		if (event_name == "" || event_start_date == "" || event_end_date == ""|| opportunityID =="0") 
   		{
   		...
   		$.ajax({
   			type : "POST",
   			url : "opportunityEvent?tabid=saveCustomerEvent",
   			data : "data=" + obj,
   			success : function(data) {
...
function update_event()
{
			...
			var opportunityID=$("#event_opportunityID").val();
			...
			$.ajax({
					type : "POST",
					url : "opportunityEvent?tabid=updateCustomerEvent",
				    data :"data=" + obj,
				    success : function(data)
				    {
				     $('#update_event_modal').modal('hide'); 
