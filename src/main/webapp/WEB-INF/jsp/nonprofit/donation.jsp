...
<head>
	...
	<title>
		<spring:message code="BzComposer.nonprofit.donationtitle" />
	</title>
	<style type="text/css">
	...
	<div id="cos">
		<div class="statusquo ok">
			<div id="hoja">
				<div id="blanquito">
					<div id="padding">
						<form:form name="DonationForm" id="donationForm" method="post"
											...
											<tr>
												...
												<td align="center"
													...
													<input type="button" class="formbutton"
														onclick="getSalesOrderDetailsByBtnName(this.form, 'FirstDonation');"
														style="padding: 8px 10px 8px 10px; font-size: 16px;"
														value='<spring:message code="BzComposer.global.first" />' />
													<input type="button" class="formbutton"
														onclick="getSalesOrderDetailsByBtnName(this.form, 'LastDonation');"
														style="padding: 8px 10px 8px 10px; font-size: 16px;"
														value='<spring:message code="BzComposer.global.last" />' />
													<input type="button" class="formbutton"
														onclick="getSalesOrderDetailsByBtnName(this.form, 'PreviousDonation');"
														style="padding: 8px 10px 8px 10px; font-size: 16px;"
														value='<spring:message code="BzComposer.global.previous" />' />
													<input type="button" class="formbutton"
														onclick="getSalesOrderDetailsByBtnName(this.form, 'NextDonation');"
														style="padding: 8px 10px 8px 10px; font-size: 16px;"
													...
													<table>
														<tr>
															...
															<td align="left"
																style="font-size: 14px;">
																<spring:message
																	code="BzComposer.nonprofit.donation.donationNo" />
															</td>
														</tr>
														<tr>
															<td style="font-size: 14px;">
																<form:select path="templateType">
																	<form:option value="1">Donation Standard</form:option>
																	<form:option value="2">Donation Charcoal</form:option>
																</form:select>
															</td>
															...
															<td style="font-size: 14px;">
																...
																<img src="${pageContext.request.contextPath}/images/cal.gif"
																	onclick="displayCalendar(document.DonationForm.orderDate,'mm-dd-yyyy',this);">
															</td>
															<td align="right"
																style="font-size: 14px;">
																<form:input path="donationNo"
																	style="text-align: right;"
																	id="donationNo" size="10"
																	readonly="true" />
															</td>
														</tr>
													</table>
													...
													<table>
														<tr>
															<td id="bill_id"
																style="font-size: 14px;">
																<spring:message
																	code="BzComposer.nonprofit.donation.donor" />
																<br />
										...
										<table class="tabla-listados" cellspacing="0">
											...
											<tr>
												<td id="td1" style="font-size: 14px;padding-left: 8px;">
													<spring:message code="BzComposer.nonprofit.donation.item" />
												</td>
												<td colspan="3" style="font-size: 14px;">
													<spring:message code="BzComposer.nonprofit.donation.description" />
												</td>
												<td style="font-size: 14px;">
													<spring:message code="BzComposer.nonprofit.donation.classification" />
												</td>
												<td style="font-size: 14px;">
													<spring:message code="BzComposer.nonprofit.donation.Amount" />
												</td>
											...
											<tr>
												...
												<td style="font-size: 14px;width: 18%;">
													<div id="itemDiv" style="padding-left: 8px;">
											...
											<thead>
												<tr>
													...
													<th style="font-size: 14px;width: 545px;">
														<div id="it4">
															<spring:message
																code="BzComposer.nonprofit.donation.description" />
														</div>
													</th>
													<th style="font-size: 14px;width: 218px;">
														<div id="it5">
															<spring:message
																code="BzComposer.nonprofit.donation.classification" />
														</div>
													</th>
...
<script>
	...
	document.addEventListener("DOMContentLoaded", function() {
        ...
        for(i=0;i<size;i++){
			...
			const name = classificationId; //getClassificationNameById(classificationId);
...
<script type="text/javascript">
	...
	function clearShippingCol() {
		...
		document.DonationForm.shipping.value = "0.00";
		sumShippingTotal();
	...
	function sumShippingTotal() {
		document.DonationForm.subtotal.value = parseFloat(document.DonationForm.subtotal.value).toFixed(2);

		var convertSubData = parseFloat(document.DonationForm.subtotal.value) + parseFloat(document.DonationForm.tax.value) + parseFloat(document.DonationForm.shipping.value);
		document.DonationForm.total.value = parseFloat(convertSubData).toFixed(2);

		var convertSubData2 = parseFloat(document.DonationForm.total.value) - parseFloat(document.DonationForm.discount.value);
		document.DonationForm.adjustedtotal.value = parseFloat(convertSubData2).toFixed(2);

	}

	function clearDiscountCol() {

		document.DonationForm.adjustedtotal.value = parseFloat(document.DonationForm.total.value).toFixed(2);
		document.DonationForm.discount.value = "0.00";
	...
	function saveOldValue() {
		...
		document.getElementById('oldValue').value = document.getElementById('amount_id').value;


	}
	//this function for calculat discount amount
	function calDiscountTotal() {
		if (parseFloat(document.DonationForm.discount.value) > 0) {
			var convertSubData = parseFloat(document.DonationForm.total.value) - parseFloat(document.DonationForm.discount.value);
			document.DonationForm.adjustedtotal.value = parseFloat(convertSubData).toFixed(2);
			document.getElementById('adjustedtotal_TextField').innerHTML = "" + document.DonationForm.adjustedtotal.value;
		}
	}
	//this function for calculate adjustedtotal amount
	function caladjustedtotal() {
		var convertSubData = parseFloat(document.DonationForm.total.value);
		document.DonationForm.adjustedtotal.value = parseFloat(convertSubData).toFixed(2);
	}
	...
	function Assignment(value, form) {
		if (value == 0) {
			document.DonationForm.billTo.value = "";
			document.DonationForm.taxable.checked = false;
		}
		else {
	...
	function productTable(size) {
		...
		for (j = 0; j < value; j++) {
			...
			document.getElementById(j + '7').style.visibility = 'hidden';
	...
	function Init() {
		...
		var initPending = '${DonationForm.isPending}';
		if (initPending == 'true') {
...
debugger;
							...
							document.getElementById('tabid').value = "SaveDonation";
							document.forms['donationForm'].action = "NonProfit?tabid=SaveDonation";
	...
	function NewDonation() {
		window.location.href = "NonProfit?tabid=Donation";
	}
	function SendMail(form) {
		cid = form.donationNo.value;
		window.open("Invoice?tabid=ShowEmail&OrderType=Donation&OrderNo=" + cid, null, "scrollbars=yes,height=500,width=900,status=yes,toolbar=no,menubar=no,location=no");
	}
...
debugger;
					...
					for (jj = 0; jj < size; jj++) {
						...
						if (d == rowId) {
							...
							total = amt;
							document.DonationForm.total.value = total.toFixed(2);
	...
	function printCustomerOrder(form) {
		...
		let donationNo = form.donationNo.value;

		if (cid == 0 || cid === "") {
			return showValidationDialog(); // Make sure this function is defined elsewhere
		} else {
			let url = "NonProfit?tabid=PrintDonation&custID=" + encodeURIComponent(cid) + "&donationNo=" + encodeURIComponent(donationNo);
			window.open(
				url,
...
<div id="saveDonation" style="display: none;">
	<p>
		<spring:message code="BzComposer.nonprofit.saveorupdateDonation" />
	</p>
</div>
