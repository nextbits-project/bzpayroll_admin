...
var marr= new Array(31,28,31,30,31,30,31,31,30,31,30,31);
...
Epoch.prototype.reDraw = function () //PRIVATE: reapplies all the CSS classes for the calendar cells, usually called after chaning their state
{
	...
	seldate=this.selectedDates[0];
	weekno=seldate.getWeek();
...
function getDate123()
{
	  ...
	  if(year%4==0)
	    ...
	    if(temp<0)
	    {
	         ...
	         var st=marr[tmonth-1]+temp+1;
	         if(tmonth<10)
	            dmonth="0"+tmonth;
	         else
	            dmonth=""+tmonth;
	         
	         for(cnt2=st;cnt2<=marr[tmonth-1];cnt2++)
	         {
	               if(cnt2<10)
	                ddate="0"+cnt2;
	               else
	                  ddate=""+cnt2;
	               weekdays[cnt1]=dmonth +"-" +ddate + "-" + tyear;
	         ...
	         if(tmonth<10)
	            ...
	            dmonth=""+tmonth;   
	         var con = 7-cnt1;
	         for(cnt3=1;cnt3<=con;cnt3++)
	         {
	             if(cnt3<10)
	               ddate="0"+cnt3;
	             else
	                  ddate=""+cnt3; 
	             weekdays[cnt1]=dmonth +"-" +ddate + "-" + year;
	             cnt1=cnt1+1;
	    ...
	    else
	    {
	       ...
	       if(temp1 >marr[month])
	       {
	            ...
	            dmonth=""+tmonth;
	       		for(cnt4=date-day+1;cnt4<=marr[month];cnt4++)
	            {
	              if(cnt4<10)
	               ddate="0"+cnt4;
	             else
	       ...
	       for(fcnt=0;fcnt<7;fcnt++)
	      {
	         var id="d"+fcnt;
	         var tddata= document.getElementById(id);
	         tddata.innerHTML=weekdays[fcnt];
	         var row=fcnt+1;
	         setRowDates(row);
